...
### ⚡ Параллельный запуск при таймауте

Если агент вернул **Request timed out** — не жди повторного ответа последовательно.  
Вместо этого: **запусти ту же задачу сразу 2–3 агентам параллельно** через `run_parallel_agents`.  
Кто ответит первым — тот результат и использовать. Остальные — игнорировать.

```
Пример: Gemini timeout →
run_parallel_agents([
  {"agent": "gemini", "prompt": "..."},
  {"agent": "qwen",   "prompt": "..."},
  {"agent": "hermes", "prompt": "..."}
...
Это быстрее чем ждать восстановления одного агента.
